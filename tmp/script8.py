...
Arquivo: enem_ita_semiguais_<ano>.csv
Salva os nomes das colunas restantes em colunas_interesse_ita_semiguais_enem.csv
'''

print("ANO\tIGUAIS")
unicos = {}
for ano in [2018, 2019, 2022, 2023]:
    df = pd.read_csv(f"enem_ita_todas_{ano}.csv", encoding='ISO-8859-1', delimiter=';', low_memory=False)
...
for ano in [2018, 2019, 2022, 2023]:
    ...
    df_sem_colunas.to_csv(f"enem_ita_semiguais_{ano}.csv", index=False, sep=';')

print("Colunas restantes:", len(df_sem_colunas.columns))
print("Nomes salvos em colunas_interesse_ita_semiguais_enem.csv")
df = pd.DataFrame(df_sem_colunas.columns, columns=["Nome"])
df.to_csv("colunas_interesse_ita_semiguais_enem.csv", index=False)